...
1) What value of m in (3.11) is taken in the concrete numerical evaluations?\\
 \noindent {\it Reply\\
 In the analysis we have taken $m=6$. We also added a corresponding note to the paper in
 the captions of Fig.(1) and Fig.(3) in Sec.(6).
...
2) Nearly all relations in the paper are given in an implicit way, (for example keeping derivatives of the superpotential unspecified). In my opinion, the paper would improve if it showed the following explicit results:\\
 ...
 However, we are happy to make them explicit immediately after Eq. (3.12) which we have done in the two unnumbered equations 
 after Eq.(3.12) in the revised version of the paper.
 Similarly we have added a line after \bl{Eq.(3.7)} directing the reader to section 8 (an appendix) where the covariant derivatives
 are defined.
 }
...
2.1) A check that the real directions $\rho_1$  and  $\rho_2$ are indeed stable, also in presence of $W_{sb}$ and of subleading terms in
 ...
 stability condition to all orders in the $1/T$ expansion as can be seen from Eq. (3.17). We have also checked numerically  that 
...
2.2) A similar analysis should be done for the combination  $a_+$. After eq. (3.26), the paper states "$a_+$ undergoes fast roll.... thus we suppress $a_+$". This point should be clarified. My understanding is that this sentence means that $a_+$ has a much larger mass than $a_-$, so we can consistently set $a_+$ = 0. This should be shown explicitly, by computing explicitly the mass of this field (in terms of the parameters in the model) and showing that it is indeed greater than H.\\

\noindent {\it Reply\\
This is one of the central points of the analysis, i.e., decomposition of the potential into slow roll vs fast roll where the slow roll potential
 ...
 and Fig.2 of JHEP {\bf 1803}, 121 (2018).
However, we are happy to elaborate on this point in this work. The mass of $a_+$ is controlled largely by $W_s$ and $W_{sb}$ makes
 significantly smaller contribution to $a_+$. In the analysis we choose $\mu=M_P$ where $M_P$ is the Planck mass while
  the mass scales that enter $W_{sb}$ are significantly smaller than $M_P$.
Thus to a good approximation the mass of $a_+$ is controlled by $\mu$ alone and a simple analysis using $W_s$ shows that
$m_{a_+}\simeq 2 \sqrt 2 \mu$. We have checked  that $m_{a_+}\simeq 2 \sqrt 2 \mu$ is larger than $H$
for the entire parameter space we investigate.
We note that $\mu$ does not enter in the $a_-$ mass as can be seen in Eqs.~(3.31)-(3.34) and
choose parameter space of $W_{sb}$ such that $m_{a_-}$ is smaller that $m_{a_+}$. We exhibit these results graphically in Fig.~(5) in the
revised paper. Here Fig.~(5a) is a histogram of $H$ values which shows that $m_{a_+}$ is always larger than $H$.
Fig.~(5b) is a histogram of $m_{a_-}$ values which shows that $m_{a_-}$ is always significantly smaller than $m_{a_+}$ for the parameter space chosen.
In addition to Fig.~(5)
we have added a further discussion of this in text in the revised version of the paper
to explain this point further. This is done at the end of the paragraph after Eq.~(3.27).
}
...
analysis. However, the potential by itself is not the whole story in the analysis for the Dirac-Born-Infeld case. Here the entire
...
that our references to the analysis of non-Gaussianity from the Planck experiment needed updating. We have
done so in the revised version.  The changes made appear after Eq.(5.12) and near the end of section 6. We have included
...
\noindent {\it Reply\\ 
We agree with the referee  regarding the conventional way to find  the end of inflation.
For the Dirac-Born-Infeld the numerical integration is time intensive especially for the very large scan
involving $0.5\times 10^6$ parameter sets that we carried out.  For this reason we approximated the end of inflation the way we did
which is much faster to implement than the $\ddot {a}=0$ constraint. However, we have taken time to 
carry out the analysis using the $\ddot{a}=0$ constraints and our analysis shows that the results of our approximate
procedure and the conventional ones agree for 97\% of the parameter points. In any case we have replaced the old 
Figs (1) and (2) with new ones using $\ddot {a}=0$ constraint. Further, we have replaced Eq.(6.1) with the new constraint.
}
...
\noindent {\it Reply\\
We agree that the distribution used to generate the dataset from Figs.(1, 2) is important to be
mentioned in the paper. We added a complete specification to the caption of Fig.(1).
Since $a_-$ is a pNGB, $\mu$ and $\lambda$ do not enter in the slow-roll Lagrangian, as can be seen in Eqs.~(3.30) to (3.35).
}
...
\noindent {\it Reply\\
We agree with the suggestion of the referee. We have added a new figure 4 and a short discussion of it at the end of section 6 where we have presented a scatter plot of $f / M_\text{P}$ vs $n_s$ and $f / M_\text{P}$ vs $r$ for the data sets of figures 1 and 2. Here we see that $f$
 lies in the sub-Planckian region for a signifiant set of parameter points.}\\

{\it In the above we have replied to all of the comments and suggestion of the referee. We have revised the manuscript to
take account of them. We are submitting the revised manuscript  for publication in JHEP.}
