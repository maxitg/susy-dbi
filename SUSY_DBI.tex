...
\section{Supersymmetric DBI action for two chiral fields \label{sec3}}
  ...
  \begin{align}
    ...
    \label{Lag.22}
  \end{align}
  \pn{where the supercovariant derivatives $D^\alpha \Phi$ etc are defined in an explicit manner in section 8.}  
  ...
  \begin{align}
    ...
    \label{Lag.23}
  \end{align}
  \pn{where $\frac{\partial W}{\partial \phi_k}$ (k=1,2) are given by 
  \begin{align}
...
Naively, it appears there are three solutions for $F_k$. However, as exhibited in section \ref{appenB}, only $j = 0$ is a solution to the full Euler-Lagrange equations for $F$.
  ...
  \end{equation}
  \pn{Eqs.(\ref{DisplayFormulaNumbered:eq.twoDBI.stabilization.1}) and 
...
%  \begin{align}
%  f\equiv f_1=f_2= -\frac{2\mu}{\lambda}  - \sum_k \sum_{k=1}^m \frac{1}{2^{k/2-1}} k A_{k}  (\frac{-2\mu}{\lambda})^{k-1}
%  /\left[1+\frac{3}{2}(\frac{-2\mu}{\lambda}\right]  +\cdots
%  \end{align}
    \begin{align}
  ...
  \pn{This topic was dealt with in great detail in \cite{Nath:2017ihp}. Here the more general case of $m$ pairs of axion fields
...
\section{Slow roll parameters and non-Gaussianity \label{sec5}}
  ...
  \end{align}
  where $P_k^{\zeta}$ is the scalar power spectrum and $\pn{f_{NL}}$ is a measure of non-Gaussianity. 
  \pn{Non-Gaussianity depends strongly on  shape and various types of shapes have been discussed in the 
...
\cite{Komatsu:2001rj,Komatsu:2003fd,Verde:1999ij}).
 ...
 % of Eq. (\ref{equil}) is referred to in the
%literature as the equilibrium one which is indicated at the superscript ``equil''.}
  when $k_1=k_2=k_3$ and  is given by~\cite{Chen:2006nt}}
  \begin{align}
   \pn{f^{equil}_{NL}}= \frac{35}{108}\left(\frac{1}{c_s^2}-1\right) - \frac{5}{81} \left[ \left(\frac{1}{c_s^2} -1- 2\frac{z_2}{z_1}\right) + (3-2 {c}_1)z \frac{z_2}{z_1}\right] + {\cal O}(\epsilon).
    \label{equil}
  \end{align}
  where
  \begin{align}
  ...
  \end{align}
 \pn{where the superscript ``equal''  on $f_{NL}$ indicates that we are specifically considering the shape to be of an equilateral 
 triangle. }
  
  
%\pn{
%Non-Gaussianity depends strongly on the shape and various types of shapes have been discussed in the 
%literature which include local, equilibrium and orthogonal ~\cite{Ade:2015ava} (For earlier analyses see 
%\cite{Komatsu:2001rj,Komatsu:2003fd,Verde:1999ij}).
% The non-Gaussianity of Eq. (\ref{equil}) is referred to in the
%literature as the equilibrium one which is indicated at the superscript ``equil''.}
  ...
  \begin{figure}
    ...
    \caption{
      ...
      are consistent with Planck constraints as shown by green points while the red points lie outside the 68\% CL contour.
            Right panel: A plot of non-Gaussianity parameter $\pn{f^{equil}_{NL}}$ as
      ...
      %The horizontal black line is the
 %lower limit $\pn{f^{equil}_{NL}} =5$ of the 
  ...
  \pn{Max: We need to remove the horizontal black line.}  
    }
    \label{fig1}
  \end{figure}
  ...
  \begin{figure}
    ...
    $\pn{f^{equil}_{NL}}$ is plotted vs $N_{\rm pivot}$.
...
Numerical simulations performed consist of two parts: a large $0.5 \times 10^6$ points simulation in which $\alpha_2 = \alpha_3 = 0$, and a small simulation with $\alpha_2$ and $\alpha_3$ perturbed around an experimentally consistent point with the largest non-Gaussianity from the first simulation.
 ...
 speed of sound $c_s$ 
  ...
  The right panel of Fig. (\ref{fig1}) gives a plot of $\pn{f^{equil}_{NL}}$ vs $\alpha_1$. We see a significant sensitivity
  of $\pn{f^{equil}_{NL}}$ to $\alpha_1$. However, overall $\pn{f^{equil}_{NL}}$ is typically small and mostly lies below 0.5. The effect of $\alpha_2$ and $\alpha_3$ for the point from Fig.~\ref{fig1} with largest non-Gaussianity is shown on Fig.~\ref{fig3}. One can see that although non-Gaussianity $f_\text{NL}$ is sensitive to $\alpha_2$ and $\alpha_3$, it still lies below 0.5. This level
  of non-Gaussianity appears too low for observation in the near future. 
  
  \pn{The most recent analyses of non-Gaussianity based on Planck data are given in ~\cite{Ade:2015ava}. 
...
%  The analysis of~\cite{Komatsu:2001rj,Komatsu:2003fd,Verde:1999ij}
%  indicates that non-Gaussianity can be tested in data in future experiments provided $|\pn{f^{equil}_{NL}}|> 5$ and the $\pn{f^{equil}_{NL}}$
%  given by the right panel of Fig. (\ref{fig1}) lies significantly below that. In the analysis we allow a corridor of $[50,60]$
%  e-foldings and it is of interest to ask the dependence of $n_s$ and $\pn{f^{equil}_{NL}}$ on this number.
  ...
  The right panel gives a plot of $\pn{f^{equil}_{NL}}$ on $N_{\rm pivot}$. Here one finds the $\pn{f^{equil}_{NL}}$ has a relatively small variation in the range $[50,60]$ for
